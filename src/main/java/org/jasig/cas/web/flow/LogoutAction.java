...
import com.gionee.cas.util.EoaConfiguration;
...
public final class LogoutAction extends AbstractLogoutAction {
    ...
    private static final String OLD_CAS_LOGOUT_PREFIX = "redirect";
    ...
    protected Event doInternalExecute(final HttpServletRequest request, final HttpServletResponse response,
        ...
        boolean isAjax = StringUtils.hasText(header) && "XMLHttpRequest".equals(header);
        if (!isAjax) {
            service = request.getParameter(CasProtocolConstants.PARAMETER_SERVICE);
            // for system who used old cas
            if (service == null) {
                service = request.getParameter(OLD_CAS_LOGOUT_PREFIX);
                if (service == null) {
                    service = EoaConfiguration.getEoaUrl();
                } else {
