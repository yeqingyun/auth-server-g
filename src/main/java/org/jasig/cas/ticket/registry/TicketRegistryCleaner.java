...
import com.gionee.cas.ticket.RedisTicketOperationTemplate;
import com.gionee.cas.ticket.RedisTicketRegistry;
...
public class TicketRegistryCleaner implements Job {
    ...
    public void execute(final JobExecutionContext jobExecutionContext) throws JobExecutionException {
        ...
        try {
            ...
            if (registry instanceof RedisTicketRegistry) {
                cumulativeCount = doTicketCleanOperationWithRedisScan(registry);
            } else {
                final Collection<Integer> deletedTicketCounts = doTicketClean(registry);
    ...
    private Collection<Integer> doTicketClean(final TicketRegistry registry) {
        Collection<Ticket> registryTickets = registry.getTickets();
        return Collections2.transform(registryTickets, new Function<Ticket, Integer>() {
            @Override
    ...
    private Integer doTicketCleanOperationWithRedisScan(final TicketRegistry registry) {
        RedisTicketRegistry redisTicketRegistry = (RedisTicketRegistry) registry;
        return redisTicketRegistry.doInRedisScan(new RedisTicketOperationTemplate() {
            @Override
            public boolean doInRedisScan(final Ticket ticket) {
                boolean isCleanSuccessful = false;
                if (ticket != null && ticket.isExpired()) {
                    if (ticket instanceof TicketGrantingTicket) {
                        logger.debug("Cleaning up expired ticket-granting ticket [{}]", ticket.getId());
                        logoutManager.performLogout((TicketGrantingTicket) ticket);
                        registry.deleteTicket(ticket.getId());
                        isCleanSuccessful = true;
                    } else if (ticket instanceof ServiceTicket) {
                        logger.debug("Cleaning up expired service ticket [{}]", ticket.getId());
                        registry.deleteTicket(ticket.getId());
                        isCleanSuccessful = true;
